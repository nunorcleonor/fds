...
NAMELIST /MISC/ AGGLOMERATION,AEROSOL_AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,&
                ...
                C_WALE,DNS,DO_IMPLICIT_CCREGION,DRAG_CFL_MAX,DEPOSITION,ENTHALPY_TRANSPORT,&
                ...
                IBLANK_SMV,IMMERSED_BOUNDARY_METHOD,&
                ...
                PR,PROCESS_CUTCELLS,PROJECTION,P_INF,PROCESS_ALL_MESHES,RAMP_GX,RAMP_GY,RAMP_GZ,&
...
NAMELIST /DUMP/ CFL_FILE,CLIP_RESTART_FILES,COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,&
                ...
                SMOKE3D_QUANTITY,SMOKE3D_SPEC_ID,STATUS_FILES,SUPPRESS_DIAGNOSTICS,T_DEVC_LINE_END,UVW_TIMER,&
                VELOCITY_ERROR_FILE,WRITE_XYZ,GET_CUTCELLS_VERBOSE
...
IF (SMOKE3D) THEN
   QUANTITY_LOOP: DO N=1,4
      CALL GET_QUANTITY_INDEX(SMOKE3D_SMOKEVIEW_LABEL(N),SMOKE3D_SMOKEVIEW_BAR_LABEL(N),SMOKE3D_QUANTITY_INDEX(N),I_DUM(1), &
                              ...
                              SMOKE3D_QUANTITY(N),'null',SMOKE3D_SPEC_ID(N),'null','null','null','null','null')
   ENDDO QUANTITY_LOOP
ENDIF
...
INTEGER  :: N_SPEC_READ,N,NN,NNN,NS2,NR,N_SPEC_READ_2,N_SUB_SPECIES,NS,N_BINS,N_COPY,N_FOUND,MINT_SPECIES
...
NAMELIST /REAC/ A,AUTO_IGNITION_TEMPERATURE,C,CHECK_ATOM_BALANCE,CO_YIELD,CRITICAL_FLAME_TEMPERATURE,&
                ...
                ID,IDEAL,LOWER_OXYGEN_LIMIT,N,NU,N_S,N_T,O,PRIORITY,RADIATIVE_FRACTION,RAMP_CHI_R,REAC_ATOM_ERROR,&
...
NAMELIST /RADI/ ANGLE_INCREMENT,BAND_LIMITS,C_MAX,C_MIN,INITIAL_RADIATION_ITERATIONS,KAPPA0,NMIEANG,NUMBER_RADIATION_ANGLES,&
                PATH_LENGTH,QR_CLIP,RADIATION,RADIATION_ITERATIONS,RADTMP,RTE_SOURCE_CORRECTION,TIME_STEP_INCREMENT,&
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
   ...
   QUANTITY_IF: IF (DV%QUANTITY /= 'null') THEN
      CALL GET_QUANTITY_INDEX(DV%SMOKEVIEW_LABEL,DV%SMOKEVIEW_BAR_LABEL,QUANTITY_INDEX,I_DUM(1), &
                              DV%Y_INDEX,DV%Z_INDEX,DV%PART_CLASS_INDEX,DV%DUCT_INDEX,DV%NODE_INDEX(1),DV%REAC_INDEX,DV%MATL_INDEX,&
                              'DEVC', &
   ...
   QUANTITY2_IF: IF (TRIM(DV%QUANTITY2)/='null') THEN
      ...
      DV%SMOKEVIEW_BAR_LABEL = TRIM(OUTPUT_QUANTITY(DV%OUTPUT_INDEX)%SHORT_NAME)//'_'// &
                               TRIM(OUTPUT_QUANTITY(DV%OUTPUT2_INDEX)%SHORT_NAME)//'_'//TRIM(DV%STATISTICS)
...
IF (TRIM(QUANTITY)=='HRRPUV REAC') THEN
   IF (TRIM(REAC_ID)=='null') THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Output QUANTITY HRRPUV_REAC requires a REAC_ID'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   DO NR = 1,N_REACTIONS
